...
function render(gameBoard, gameState) {
  gameState.forEach((row, rowIndex) => {
    row.forEach((column, columnIndex) => {
      column.style.top = `${rowIndex * 475}px`;
      column.style.left = `${columnIndex * 475}px`;

      column.style['background-position-y'] = `-${rowIndex * 475}px`;
      column.style['background-position-x'] = `-${columnIndex * 475}px`;
